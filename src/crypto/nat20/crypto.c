...
void n20_memzero32(void volatile* dst, size_t n) {
    // Use volatile to prevent compiler optimizations that might skip the memset
    uint8_t volatile* p = dst;
    switch (((uintptr_t)p) & 0x3) {
        case 1:
            *p++ = 0;
            FALLTHROUGH
        case 2:
        ...
        case 3:
            *p++ = 0;
        case 0:
            break;  // Align to 4-byte boundary
    }
    n &= ~0x3;  // Clear the last 1-3 bytes if n is not a multiple of 4
    uint32_t volatile* p32 = (uint32_t volatile*)p;
    while (n >= sizeof(uint32_t)) {
        *p32++ = 0x0;
        n -= sizeof(uint32_t);
    }
    p = (uint8_t volatile*)p32;
    switch (n) {
        case 3:
            *p++ = 0;
            FALLTHROUGH
        ...
        case 1:
            *p++ = 0;
            FALLTHROUGH
        case 0:
            break;  // No more bytes to clear
    }
}

void n20_memzero(void volatile* dst, size_t n) {
    // Use volatile to prevent compiler optimizations that might skip the memset
    uint8_t volatile* p = dst;
    switch (((uintptr_t)p) & 0x7) {
        case 1:
            *p++ = 0;
            FALLTHROUGH
        case 2:
        ...
        case 7:
            *p++ = 0;
            FALLTHROUGH
        case 0:
            break;  // Align to 8-byte boundary
    }
    n &= ~0x7;  // Clear the last 1-7 bytes if n is not a multiple of 8
    uint64_t volatile* p64 = (uint64_t volatile*)p;
    while (n >= sizeof(uint64_t)) {
        *p64++ = 0x0;
        n -= sizeof(uint64_t);
    }
    p = (uint8_t volatile*)p64;
    switch (n) {
        case 7:
            *p++ = 0;
            FALLTHROUGH
...
n20_error_t n20_hmac(n20_crypto_digest_context_t* ctx,
    ...
    if (mac_size_in_out == NULL || *mac_size_in_out < digest_size) {
    ...
    rc = ctx->digest(ctx, alg_in, msg, 2, mac_out,
...
n20_error_t n20_hkdf(n20_crypto_digest_context_t* ctx,
    ...
    n20_error_t error = ctx->hkdf_extract(ctx, alg_in, key, salt, prk, &prk_size);
    if (error != n20_error_ok_e) {
        return error;
    }

    n20_error_t rc = ctx->hkdf_expand(ctx, alg_in, (n20_slice_t){prk_size, prk}, info, key_octets, out);
...
n20_error_t n20_hkdf_extract(n20_crypto_digest_context_t* ctx,
    ...
    ctx->hmac(ctx, alg_in, salt, &ikm_list, prk, &digest_size);
